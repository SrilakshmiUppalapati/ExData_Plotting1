...
globalReactivePower <- as.numeric(workingDataSet$Global_reactive_power)
voltage <- as.numeric(workingDataSet$Voltage)
subMetering1 <- as.numeric(workingDataSet$Sub_metering_1)
subMetering2 <- as.numeric(workingDataSet$Sub_metering_2)
subMetering3 <- as.numeric(workingDataSet$Sub_metering_3)
...
# Close device so that the plot is flushed to file
dev.off()
